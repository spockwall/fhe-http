...
use crate::configs::typing::WasmProvenFheType;
...
macro_rules! impl_binary_py_fhe_ops {
    ($func_name:ident, $method:ident) => {
        pub fn $func_name(
            ...
            if let Ok(res) = ($method)(&a, &b, &data_type, &public_zk_param, &public_key) {
                Ok(res)
            } else {
...
macro_rules! impl_unary_py_fhe_ops {
    ($func_name:ident, $method:ident) => {
        pub fn $func_name(
            ...
            if let Ok(res) = ($method)(&a, &data_type, &public_zk_param, &public_key) {
                Ok(res)
            } else {
                Err(JsValue::from_str(&format!(
                    "{}",
